...
def Automata(cadena,Lenguaje,Estado,iterador,Tabla,historial):
    flag=True
    historialep=historial
    #EL programa verifica si tiene transicciones epsilon
    if((len(Tabla[int(Estado)][int(len(Lenguaje.lexemas)-1)]))>=1):
        historialep=historialep+"(EP)->"
        y=int(len(Lenguaje.lexemas)-1)
        x=int(Estado)
        Transiciones=len(Tabla[x][y])
        for i in range(0,Transiciones):
            Automata(cadena,Lenguaje,Tabla[x][y][i],iterador,Tabla,historialep + "q"+Tabla[x][y][i] )
            flag=False 
    #El programa verifica las demas Transiciones
    if(len(cadena)>iterador):
        if(cadena[iterador] in Lenguaje.lexemas):
            ...
            historial=historial + "-EM("+cadena[iterador]+")->q"
            Automata(cadena,Lenguaje,Estado,iterador+1,Tabla,historial+ str(Estado))
            flag=False   
    if (len(cadena)==iterador):
        if(Estado in Lenguaje.final):
            ...
            Lenguaje.flag=True
            flag=False
    #Si la bandera no pudo ser apaga esto significara que no hay mas transiciones     
    if flag:
        if(Estado in Lenguaje.final and len(cadena)==iterador):
            Lenguaje.cadenas.append(historial)
            Lenguaje.flag=True
...
def main():#Abrimos archivo y tratamos datos para poder ser tratados
    ...
    #agreamos la transicion Epsilon
    Lenguaje.lexemas.append('E')
