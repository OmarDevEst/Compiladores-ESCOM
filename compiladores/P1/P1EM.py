...
def LLenarTabla (Tabla,Lenguaje):
    ...
    for i in range(0,len(Lenguaje.estados)+1):
        for j in range(0,len(Lenguaje.lexemas)):
            if len(Tabla[i][j]) == 0:
                Tabla[i][j].append(str(len(Lenguaje.estados)))
    return Tabla
            #a     #b      #c
# tabla = q0[[0,1][0][]
#          q1[null][2]
...
def main():#Abrimos archivo y tratamos datos para poder ser tratados
    ...
    Tabla= [[[] for x in range(len(Lenguaje.lexemas))] for y in range(len(Lenguaje.estados)+1)] 
